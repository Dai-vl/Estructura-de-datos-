...
//2ª FORMA: ahora el mapa del mapa de personas esta ordenado, no se ordena luego el vector
...
private:
    unordered_map<string, map<string, list<string>::iterator>> maP;
    unordered_map<string, list<string>> maE; //front sera la mas antigua
public:

    void altaOficina(string p, string e) {
        if (!maP.count(p) || (maP.count(p) && !maP.at(p).count(e))) { //coste logaritmico en el numero de empleos que tenga la persona
            maE[e].push_back(p);
    ...
    vector<string> listadoEmpleos(string p) { //coste lineal en el numero de empleos que tiene la persona
